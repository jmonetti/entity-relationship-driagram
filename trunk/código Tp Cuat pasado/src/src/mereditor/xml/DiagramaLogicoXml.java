...
public class DiagramaLogicoXml extends DiagramaControl implements XmlLogicizable{

	public DiagramaLogicoXml(Proyecto proyecto) {
		super(proyecto);
	}

	public DiagramaLogicoXml(Proyecto proyecto, DiagramaControl componente) {
		this(proyecto);
	...
	public Element toXml(ModeloLogicoParserXml parser) throws Exception {
		...
		if (this.diagramas.size() > 0) {
			...
			//TODO	diagramasElement.appendChild(parser.convertirXmlizable(diagrama).toXml(parser));
			}
		}

		// Agregar el resultado de la validacion
		//TODO elemento.appendChild(parser.convertirXmlizable(this.validacion).toXml(parser));
